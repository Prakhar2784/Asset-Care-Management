...
const AdminDashboard = () => {
  ...
  // Hardcoded alerts (can be moved to backend later)
  ...
  const stats = [
    {
      ...
      icon: <Inventory2Rounded sx={{ fontSize: 40 }} />,
      color: "#1E3A8A",
      route: "/admin/assets",
    },
    {
      ...
      icon: <ConfirmationNumberRounded sx={{ fontSize: 40 }} />,
      color: "#0F766E",
      route: "/tickets",
    },
    {
    ...
    },
    {
      ...
      value: dashboardData?.pendingTickets || "0",
      subtitle: "Requires HOD authorization",
      icon: <ApprovalRounded sx={{ fontSize: 40 }} />,
      color: "#D97706",
          ...
          <Button
            ...
            sx={{
              ...
              "&:hover": {
                ...
                boxShadow: "0 20px 36px rgba(15,118,110,0.36)",
      ...
      {loading ? (
        ...
        <>
            ...
            {stats.map((stat, index) => (
              <Grid item xs={12} sm={6} lg={3} key={index}>
                <Box
                  ...
                  sx={{ cursor: "pointer", height: "100%", transition: "0.3s ease", "&:hover": { transform: "translateY(-5px)" } }}
                >
                  <StatCard
                    title={stat.title}
                    value={stat.value}
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} lg={8}>
              <Paper sx={{ p: { xs: 2.5, md: 3.5 }, borderRadius: "28px", bgcolor: "#FFFFFF", border: "1px solid #E2E8F0", boxShadow: "0 16px 40px rgba(15,23,42,0.06)" }}>
                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: { xs: "flex-start", sm: "center" }, gap: 2, mb: 3, flexDirection: { xs: "column", sm: "row" } }}>
                  <Box>
                    <Typography sx={{ fontWeight: 950, color: "#0F172A", fontSize: "22px", letterSpacing: "-0.7px" }}>
                      Active Service Tickets
                    </Typography>
                    <Typography sx={{ color: "#64748B", fontSize: "14px", fontWeight: 600, mt: 0.5 }}>
                      Click any ticket to view service details and timeline.
                    </Typography>
                  </Box>
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  {recentTickets.length === 0 ? (
                    ...
                    recentTickets.map((ticket) => (
                      <Box
                        key={ticket.id}
                        onClick={() => handleTicketOpen(ticket)}
                        sx={{
                          p: { xs: 2, md: 2.5 }, borderRadius: "20px", bgcolor: "#F8FAFC", border: "1px solid #E2E8F0",
                          display: "flex", justifyContent: "space-between", alignItems: { xs: "flex-start", md: "center" },
                          flexDirection: { xs: "column", md: "row" }, gap: 2, transition: "all 0.3s ease", cursor: "pointer",
                          "&:hover": { borderColor: "rgba(15,118,110,0.35)", bgcolor: "#FFFFFF", transform: "translateY(-3px)", boxShadow: "0 14px 30px rgba(15,23,42,0.08)" },
                        }}
                      >
            ...
            {/* System Alerts & Quick Links Section */}
            <Grid item xs={12} lg={4}>
              <Paper sx={{ p: { xs: 2.5, md: 3.5 }, borderRadius: "28px", bgcolor: "#FFFFFF", border: "1px solid #E2E8F0", boxShadow: "0 16px 40px rgba(15,23,42,0.06)", height: "100%", display: "flex", flexDirection: "column" }}>
                <Typography sx={{ fontWeight: 950, color: "#0F172A", fontSize: "22px", mb: 2.5, letterSpacing: "-0.7px" }}>
                  System Alerts
                </Typography>

                {alerts.length === 0 ? (
                  <Paper sx={{ p: 3, borderRadius: "20px", bgcolor: "#F8FAFC", border: "1px solid #E2E8F0", textAlign: "center" }}>
                  ...
                  <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mb: 3 }}>
                    {alerts.map((item) => (
                      <AlertBox key={item.id} alert={item} onOpen={() => handleAlertOpen(item)} onDismiss={() => handleDismissAlert(item.id)} />
                    ))}
                  </Box>
                )}

                <Typography sx={{ fontWeight: 950, color: "#0F172A", fontSize: "20px", mb: 2, mt: "auto" }}>
                  Quick Links
                </Typography>
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
                  {quickLinks.map((link) => (
                    <Button
                      ...
                      sx={{
                        borderColor: "#E2E8F0", color: "#0F172A", justifyContent: "space-between", textTransform: "none", py: 1.4, px: 2, borderRadius: "14px", fontWeight: 800,
                        "& .MuiButton-startIcon": { color: "#0F766E" },
                        "&:hover": { bgcolor: "rgba(15,118,110,0.08)", borderColor: "rgba(15,118,110,0.35)", color: "#0F766E" },
                      }}
                    >
        ...
        {selectedTicket && (
          <>
            ...
            <DialogContent sx={{ p: 3 }}>
              ...
              <Typography sx={{ fontSize: "18px", color: "#0F172A", fontWeight: 950, mb: 2 }}>Service Flow</Typography>
              <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
                {selectedTicket.timeline.map((step, index) => (
                  <Box key={index} sx={{ p: 2, borderRadius: "16px", bgcolor: "#FFFFFF", border: "1px solid #E2E8F0", display: "flex", alignItems: "center", gap: 1.5 }}>
                    <Box sx={{ width: 32, height: 32, borderRadius: "50%", display: "grid", placeItems: "center", background: "rgba(15,118,110,0.10)", color: "#0F766E", fontWeight: 950 }}>
                      {index + 1}
                    </Box>
...
const AlertBox = ({ alert, onOpen, onDismiss }) => {
  ...
  return (
    <Paper
      sx={{
        p: 2.2, borderRadius: "18px", bgcolor: isCritical ? "#FEF2F2" : "#FFFBEB", border: isCritical ? "1px solid #FECACA" : "1px solid #FDE68A",
        cursor: "pointer", transition: "0.3s ease", "&:hover": { transform: "translateY(-3px)", boxShadow: "0 14px 28px rgba(15,23,42,0.08)" },
      }}
      onClick={onOpen}
    >
      <Box sx={{ display: "flex", justifyContent: "space-between", gap: 1 }}>
        <Box>
          <Typography sx={{ fontWeight: 950, color: isCritical ? "#991B1B" : "#92400E", fontSize: "14px", mb: 0.5, textTransform: "uppercase", letterSpacing: "0.5px" }}>
            {alert.title}
          </Typography>
          <Typography sx={{ color: isCritical ? "#7F1D1D" : "#78350F", fontSize: "14px", fontWeight: 600, lineHeight: 1.6 }}>
            {alert.message}
          </Typography>
        </Box>
...
const DialogHeader = ({ icon, title, subtitle, onClose, danger = false }) => (
  <Box sx={{ p: 3, display: "flex", alignItems: "flex-start", gap: 2 }}>
    <Box sx={{ width: 54, height: 54, borderRadius: "16px", background: danger ? "linear-gradient(135deg, #991B1B, #DC2626)" : "linear-gradient(135deg, #1E3A8A, #0F766E)", color: "#FFFFFF", display: "grid", placeItems: "center", flexShrink: 0 }}>
      {icon}
    </Box>
    <Box sx={{ flex: 1 }}>
      <Typography sx={{ fontWeight: 950, fontSize: "24px", color: "#0F172A" }}>{title}</Typography>
