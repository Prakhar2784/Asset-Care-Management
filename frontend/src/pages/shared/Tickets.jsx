  ...
  return (
    <Box sx={{ width: "100%", pb: 5 }}>
        ...
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={12} md={2}>
            <Button fullWidth variant="outlined" onClick={() => { setSearchQuery(""); setStatusFilter("All"); }}
              sx={{ height: "56px", borderColor: "#e2e8f0", color: "#64748b", borderRadius: "12px", fontWeight: 700, textTransform: "none", "&:hover": { bgcolor: "#F1F5F9", borderColor: "#CBD5E1" } }}>
              Clear Filters
            </Button>
          </Grid>
        </Grid>
              ...
              <Grid item xs={12} md={6} lg={4} key={ticket._id} component={motion.div} variants={itemVariants} layout exit={{ opacity: 0, scale: 0.9 }}>
                <Paper
                  sx={{
                    ...
                    boxShadow: "0 10px 30px rgba(15,23,42,0.04)", display: "flex", flexDirection: "column",
                ...
                >
                  ...
                  {isAdminOrHod && (
                    <Box sx={{ display: 'flex', gap: 1, mb: 2, p: 1.5, bgcolor: '#F8FAFC', borderRadius: '12px', border: '1px solid #E2E8F0' }}>
      ...
      >
        <DialogTitle sx={{ p: 0, background: "linear-gradient(135deg, #F8FAFC, #FFFFFF)" }}>
          <Box sx={{ p: 3.5, display: "flex", alignItems: "flex-start", gap: 2 }}>
            ...
            <Box sx={{ flex: 1 }}>
              <Typography sx={{ fontWeight: 900, fontSize: "24px", color: "#0F172A" }}>Raise Breakdown Ticket</Typography>
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)} PaperProps={{ sx: { borderRadius: "24px", p: 2, maxWidth: "400px", boxShadow: "0 20px 50px rgba(0,0,0,0.2)" } }}>
        ...
        <Box sx={{ p: 2, display: "flex", justifyContent: "flex-end", gap: 2 }}>
          ...
          <Button onClick={confirmDelete} disabled={deleting} variant="contained"
            sx={{ bgcolor: "#EF4444", color: "#fff", fontWeight: 800, textTransform: "none", borderRadius: "10px", boxShadow: "0 6px 15px rgba(239, 68, 68, 0.3)", "&:hover": { bgcolor: "#DC2626" } }}>
      ...
      <Dialog open={timelineOpen} onClose={handleTimelineClose} fullWidth maxWidth="md"
        PaperProps={{ sx: { bgcolor: "#FFFFFF", borderRadius: "28px", border: "1px solid #E2E8F0", boxShadow: "0 28px 70px rgba(15,23,42,0.25)", overflow: "hidden" } }}
        BackdropProps={{ sx: { backgroundColor: "rgba(15,23,42,0.55)", backdropFilter: "blur(6px)" } }}>
        {selectedTicket && (
          <>
            <DialogTitle sx={{ p: 0, background: "linear-gradient(135deg, #F8FAFC, #FFFFFF)" }}>
                ...
                {[
                  ...
                  <Grid item xs={6} md={3} key={label}>
                    <Box sx={{ p: 2, borderRadius: "16px", bgcolor: "#F8FAFC", border: "1px solid #E2E8F0" }}>
                      <Typography fontSize="12px" color="#64748B" fontWeight={800} textTransform="uppercase">{label}</Typography>
                      <Typography mt={0.5} color={color} fontWeight={900}>{value}</Typography>
                    </Box>
                  </Grid>
                ))}
              </Grid>
