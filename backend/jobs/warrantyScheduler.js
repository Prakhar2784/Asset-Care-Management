  ...
  try {
    ...
    for (const tenant of tenants) {
      await setTenantId(tenant.slug, async () => {
        ...
        // Automatically filtered by tenantId via mongoose plugin
        ...
        console.log(`[WarrantyScheduler] Checked ${expiringAssets.length} expiring assets for tenant: ${tenant.slug}.`);
      });
    }
  } catch (err) {
  ...
  try {
    ...
    for (const tenant of tenants) {
      await setTenantId(tenant.slug, async () => {
        ...
        console.log(`[AuditRetention] Purged ${result.deletedCount} audit log(s) for tenant ${tenant.slug} older than 1 year.`);
      });
    }
  } catch (err) {
