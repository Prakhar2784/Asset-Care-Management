...
const COND = {
  ...
  Average: { bg: 'rgba(119, 119, 199, 0.16)', color: '#7777C7' },
...
export default function AssignedDevices() {
  ...
  const kpis = [
    ...
    { label: 'Employees',      value: uniqueEmployees,    color: '#7777C7', icon: <GroupRounded fontSize="small" /> },
    { label: 'Departments',    value: uniqueDepts,        color: '#7777C7', icon: <ApartmentRounded fontSize="small" /> },
    { label: 'Categories',     value: uniqueCategories,   color: '#7777C7', icon: <CategoryRounded fontSize="small" /> },
  ];
              ...
              {filtered.map(a => {
                ...
                return (
                  <TableRow key={a._id} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
                    ...
                    <TableCell sx={{ py: 1.5 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Avatar sx={{ width: 34, height: 34, fontSize: 13, fontWeight: 800, background: '#7777C7', color: '#FFFFFF' }}>
